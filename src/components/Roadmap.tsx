...
const timelineData = [
  { id: 1, title: "Tuesday, January 30", items: ["Social media accounts creation.", "Partnerships with influencers.", "Making some noise."] },
  { id: 2, title: "FEBRUARY 1", items: ["Whitepaper live for public.", "Partnership with Moontok.", "Start of press release."] },
  { id: 3, title: "FEBRUARY 2", items: ["Official website goes live.", "Partnership with YouTubers.", "The start of major marketing campaigns."] },
  { id: 4, title: "FEBRUARY 4", items: ["The start of Pre-sale.", "Partnership with YouTubers.", "The start of major marketing campaigns."] },
  { id: 5, title: "FEBRUARY 5", items: ["Token distribution.", "Big marketing campaigns before the launch.", "Kong trending on Twitter."] },
];

const Roadmap = () => {
  return (
    <div className="max-w-7xl mx-auto min-h-screen bg-b py-12 px-2 sm:px-6 2xl:px-0">
      <h1 className="text-center pb-48 text-5xl font-extrabold text-white uppercase tracking-tighter">Roadmap</h1>
      {/* <div className="relative"> */}
        {/* Timeline Cards */}
        <div className="relative">
          {timelineData.map((item, index) => (
            <div key={item.id} className="mb-8 flex flex-col sm:flex-row items-center ">
              <div className="w-full">{index % 2 === 0 && <TimelineCard title={item.title} items={item.items} index={index} />}</div>

              {/* Connecting Line */}
              <div className={`w-1/5 relative hidden md:block ${index % 2 === 0 ? "-left-48" : "-right-48"} w-full h-[1px] bg-white`}></div>

              <div className="w-full">{index % 2 === 1 && <TimelineCard title={item.title} items={item.items} index={index} />}</div>
            </div>
          ))}
        </div>
      {/* </div> */}
